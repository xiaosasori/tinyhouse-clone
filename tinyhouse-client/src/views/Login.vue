<template>
  <a-layout-content class="log-in">
    ...
    <ErrorBanner v-else-if="loginError" description="Sorry! We weren't able to log you in. Please try again later!" />
    ...
    <a-card class="log-in-card">
      <div class="log-in-card__intro">
        ...
        <a-typography-text>Sign in with Google to start booking available rentals!</a-typography-text>
      </div>
      <button
        class="log-in-card__google-button"
        @click="handleConsentPage"
      >
        <img
...
export default {
  ...
  setup() {
    ...
    onError(() => {
      message.error(`Sorry! We weren't able to log you in. Please try again later!`)
    })
    ...
    const {mutate, loading: loginLoading, error: loginError, onDone: loginSuccess} = useMutation(LOGIN, () => ({
      variables: {
        input: {code}
      }
    }))
